...
extract_gltf_skeleton :: proc(data: ^cgltf.data) -> (skeleton: Skeleton) {
	...
	resize(&skeleton.restPose.transforms, len(data.nodes))
	resize(&skeleton.jointNames, len(data.nodes))
	resize(&skeleton.restPose.parents, len(data.nodes))
	resize(&skeleton.invBindPose, len(data.nodes))

	// for initializing the root node to -1 to make things easier
	for &parent in skeleton.restPose.parents {
		parent = -1
	}
	...
	for &_node, index in data.nodes {
		...
		skeleton.restPose.transforms[index] = transform

		for &child in _node.children {

			skeleton.restPose.parents[get_node_id(&data.nodes, child.name)] = i32(index)
		}
	}

	skeleton.animatedPose.parents = skeleton.restPose.parents
	...
	for &_skin in data.skins {
		...
		for i: u32 = 0; i < u32(count); i += 1 {
			...
			skeleton.invBindPose[get_node_id(&data.nodes, jointName)] = la.transpose(inverseMat)
...
extract_gltf_animations :: proc(data: ^cgltf.data) -> (clips: [dynamic]Clip) {
	...
	for &_animation in data.animations {
		...
		for &_node, index in data.nodes {
			...
			clip.tracks[index].targetId = u32(index)
		}
